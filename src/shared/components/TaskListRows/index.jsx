    ...
    return (
        <>
            <tr key={user.id} className={user.eStatus === 'd' && 'deleted-user'} >
                <td>{index + 1}</td>
                <td className='blockLink' onClick={() => navigate(route.poiManagement(user.id), { state: { zone: user?.zone, ward: user?.ward } })}>{user.block_name || '-'}</td>
                ...
                <td>
                    <Button className='ButtonListRow' onClick={() => navigate(route.poiManagement(user.id), { state: { zone: user?.zone, ward: user?.ward } })}>
                        Start Survey
                    </Button>
                </td>
            </tr>
